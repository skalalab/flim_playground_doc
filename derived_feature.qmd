...
Derived features are computed during [Numerical Feature Extraction](#sec-numerical-feature-extraction), after the successful FOV results have been concatenated. The definitions are baked into the FOV metadata, so re-running extraction from a saved metadata file reproduces the same derived columns. Each definition adds one `Derived: <name>` column to the output dataset.
