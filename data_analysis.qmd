...
In univariate analyses, the selection widgets can be used to select one feature from all feature groups. Therefore, if one feature from a feature group is selected, all the other features groups will be resetted to the value `Select`. 

![](analysis_ui_shots/uni_select.png){width=40%}

In bivariate analyses, two sets of selection widgets are rendered to allow maximum flexibility (the two features can be from the same or different feature groups). Each set of selection widgets behaves like the selection widgets in the univariate analysis. The first selected feature will be hidden in the second set of selection widgets. 

![](analysis_ui_shots/bi_select.png){width=40%}

In multivariate analyses, the selection widgets can be used to select multiple features from all feature groups. A special value `All` is introduced so that the user can conveniently select all features under the feature group. If user selects `All`, all the other options will be cleared, and vice versa. 

![](analysis_ui_shots/multi_select.png){width=40%}
...
![](analysis_ui_shots/color_by.png){width=30%}
