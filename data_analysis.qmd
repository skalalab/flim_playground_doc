...
- Once users are satisfied with the analysis (visualizations and statistical modeling results), they can [export the entire analysis workflow as a self-contained Python script](#export-workflow-as-python-script)! 🎉
...
If you cannot find a certain **numerical feature** under any numerical widgets including `Uncategorized Features`, please inspect the dataset (e.g., using Excel filters) and look for non-numeric values in that column. A small number of stray non-numeric values (e.g., "--") is tolerated: if they account for **1% or less** of the column's non-empty values, FLIM Playground converts the column to numeric anyway and replaces just those cells with NaN, reporting how many were converted. Above that threshold the column is left as text and will not appear as a numerical feature, so cleaning those cells is only necessary for columns that are mostly non-numeric.
...
- `Non-numeric values converted to NaN`: reports how many values in a column were non-numeric when that column was otherwise numeric enough to recognize (see [above](#feature-recognition)). Those cells become NaN and the column is usable as a numerical feature.
...
`Except:` is the mirror image of `All`, and it is the quick way to take a whole feature group bar one or two. Say you want to check that an effect is really about lifetime and not simply a brightness difference: select `Except:` together with `intensity_sum` to keep every other feature in the group and drop that one, as in the widget below. Selecting `All` clears everything else, while selecting a feature clears `All` but keeps `Except:`, so exclusions add up as you click.
...
Often it is easier to say what to leave out than what to keep. Suppose one field of view looks like an outlier and you want to see the dataset without it — clicking the other 79 by hand is tedious. `Except:` does it in one click: select it along with the categories to drop, and everything else stays. That is the `image_name` filter in the screenshot above, set to every field of view but `MCF7_IAA_dish1...`. Because the exclusion is re-read from the data rather than frozen into a list, it keeps meaning "everything else" as you work: widen another filter, or load a dataset with new fields of view in it, and the newcomers are kept automatically.
...
![](analysis_ui_shots/filter_deselection_notice.png){width=80%}
...
- axis label size
- legend size
- whether to show the per-group count (`n`) in the legend (when enabled, the count appears on a second line below each group name, e.g. `Control` with `n=42` beneath it)
...
Numerical features extracted by [Data Extraction](#sec-data-extraction) are automatically rendered in proper FLIM notation on plot axes, legends, and hover text. For example, the column `Lifetime fit_nadh: a1` is shown as "nadh α₁ (%)", and other extracted features map to symbols such as "τ₁ (ps)", "τₘ", "τₘ,ᵢ", "χ²ᵣ", and "τ_mod". The mapping (channel + Greek/scientific symbol + unit) follows the feature naming convention introduced in [Lifetime Features (Fit)](lifetime_fit.qmd). Columns that are not recognized — identifiers, categorical features, or arbitrary user-provided columns — are displayed unchanged. The same labels are used in the [exported script](#export-workflow-as-python-script), so its figures match what you see in the app.

### Export Workflow as Python Script

At the bottom right of the interface, users can export the entire analysis workflow as a self-contained Python script. This is useful when users want to share their analysis with others, reproduce their analysis later, customize the analysis further, or integrate it into their own workflow.
The script will include all the code and analysis configuration (filters, selected numerical features, visual channels, plotting style, module-specific configurations, etc.). After exporting, users need to put the dataset in the same directory as the script and run the script to reproduce the analysis, where the plots are rendered as matplotlib figures and saved as **SVG** files for publication-ready quality.
