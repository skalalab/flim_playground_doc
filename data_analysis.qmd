...
We are not done yet 😱! FLIM Playground also provides a suite of methods to analyze and visualize the extracted data (either using the [Data Extraction](#sec-data-extraction) module or in users' own way). They are designed to be interactive and frictionless ⚡ so that users can perform hassle-free exploration of their data with, hopefully, fun 😎. Inspired by the same data [categorization](index.qmd#feature-classes) as [Data Extraction](#sec-data-extraction) does, the [Data Analysis](#sec-data-analysis) module designs an [architectural blueprint](#shared-interactive-widgets) that all in-house analysis methods build on. It is modularized so that new methods and features can be added in the future easily.  
...
## Input
Users can upload the dataset outputted from the [Data Extraction](#sec-data-extraction) directly or *their own datasets* in **csv** (Comma Separated Values) format after finishing the [interactive configuration](#sec-data-analysis-config) setup. 
...
FLIM Playground recognizes the categorical features in the uploaded dataset based on the [user-specified configuration](#sec-data-analysis-config) if the dataset is not extracted by [Data Extraction](#sec-data-extraction). Otherwise, it recognizes categorical features specified in the [Data Extraction configuration](data_extraction_config.qmd#categorical-features). 
