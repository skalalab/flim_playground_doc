...
For image inputs, the app sums the pixel decays within each cell ROI labeled by the ROI [mask](data_extraction.qmd#mask) to obtain one cell decay [@Samimi2025]. For [2D decay inputs](data_extraction_config.qmd#two-d-decay), each CSV row already supplies a cell decay, so no mask is needed. Phasor features are calculated from each cell decay.
...
The `laser_rate` is entered in **GHz** in [FOV metadata](fov_metadata.qmd#laser-frequency) or [Configuration](data_extraction_config.qmd#laser-frequency). PhasorPy expects **MHz**, so both calls multiply by 1000: `0.08` GHz becomes `80` MHz. The `reference_dye_lifetime` remains in **nanoseconds**. Using `harmonic=2` calibrates the second-harmonic coordinates at twice the laser frequency.
...
Using default phases over the 10 ns window would instead mean analysis at **100 MHz**, requiring that frequency in calibration and lifetime conversion. This is a different approach with its own limitations for real IRFs and mixtures. Release 1.13.1 retains the **80 MHz laser frequency** in this example and uses explicit phases. See the [analysis of truncation and phasor calibration](https://pmc.ncbi.nlm.nih.gov/articles/PMC7990508/) for these distinctions.
:::
