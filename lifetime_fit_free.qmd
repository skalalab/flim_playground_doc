...
But there are cases when the decay curve is truncated due to the deadtime of the system. For example, when the laser frequency is $f = 0.08$ GHz, the theoretical period is
$T = 12.5$ ns, but in practice the time window is truncated to $T = 10$ ns. In this case, the phasor coordinates of IRF and the decay curves of both harmonics are calculated using time axis $t_k = k\,(T/K)\;\;(k=0,\ldots,K-1)$ and angular frequency $\omega = 2\pi\,hf$:
