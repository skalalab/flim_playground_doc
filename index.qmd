...
<p style="text-align: center; font-size: 0.85em; color: #666; margin-top: -10px;">*Logo courtesy of Matt Stefely. The <span style="color: #D3AE4A;">yellow</span> dot is a photon sliding down a lifetime decay curve 🛝.*^[It also appears in real life: ![FLIM Playground on a black t-shirt](fp_black.png){height=90px} ![FLIM Playground on a white t-shirt](fp_white.png){height=90px} ![FLIM Playground sticker](fp_sticker.png){height=90px} ![FLIM Playground on a water bottle](fp_waterbottle.jpeg){height=90px}]</p>
:::
...
Welcome to the FLIM Playground 🥳🎉🥂! This is an interactive graphical user interface (GUI) that allows you to extract single-cell features from fluorescence lifetime imaging microscopy (FLIM) raw data ([Data Extraction](#sec-data-extraction))^[can be readily extended to other imaging modalities] and analyze extracted features or your own datasets using a built-in repertoire of methods ([Data Analysis](#sec-data-analysis)). 
...
::: {.content-visible when-format="html"}
A video of the Data Analysis section:

{{< video src="analysis_ui_shots/data_analysis.mp4" controls=true style="object-fit:cover; border-radius:12px; display:block; margin:auto;" >}}
:::
...
Due to the limitation in local file system access, extracting features from raw data is not available in the online demo. Read on to discover how FLIM Playground transforms raw fluorescence decay data into biological insight through an integrated, interactive workflow.

::: {.content-visible when-format="html"}
A video of the Data Extraction section:

{{< video src="extraction_ui_shots/data_extraction.mp4" controls=true style="object-fit:cover; border-radius:12px; display:block; margin:auto;" >}}
:::

::: {.content-visible when-format="pdf"}
The latest version of this FLIM Playground manual is available [online](https://skalalab.github.io/flim_playground_doc/). 
:::
...
### Download
Download the desktop app from [GitHub](https://github.com/skalalab/flim_playground) and double-click it to run. Releases are currently available for Windows 11, Mac OS 26, and Ubuntu 24.04. If your operating system is not either of these, you can build it yourself by following the instructions below. 
...
At [data levels](#data-levels), the data are processed to extract the feature classes in [Data Extraction](#sec-data-extraction), and the classes are used in the [Data Analysis](#sec-data-analysis) section.
...
- [Numerical Feature Extraction](#sec-numerical-feature-extraction): extracts single-cell <span style="color: #D3AE4A;">numerical features</span> based on user-selected extractors. More extractors can be integrated in the future.
  - [Calibration (C)](numerical_feature_extraction.qmd#calibration): calibrate for IRF shift or use fluorescence lifetime standard for phasor extraction
...
- [Numerical Feature Extraction](#sec-numerical-feature-extraction): extracts single-cell \textcolor[HTML]{D3AE4A}{numerical features} based on user-selected extractors. More extractors can be integrated in the future.
  - [Calibration (C)](numerical_feature_extraction.qmd#calibration): calibrate for IRF shift or use fluorescence lifetime standard
  - Alternative lifetime extractors (D): 
    - [Fitting](#sec-lifetime-fit)
    - [Phasor](#sec-lifetime-fit-free)
  - Alternative intensity-based extractors (D):
...
Both sections are built in Python and as self-contained executables ready to run on major operating systems and in browsers (**H**).
