...
# Challenges
A diverse set of [tools](https://www.phasorpy.org/docs/stable/phasor_approach/#software) — both open-source and commercial, ranging from libraries to code-free graphical user interfaces (GUIs) — are available to extract and analyze FLIM data, providing alternative methods and therefore flexibility to FLIM researchers. For example, [PhasorPy](https://www.phasorpy.org/) [@phasorpy25] is a Python library that provides a set of functions that extract pixel-level phasor features and analyze fluorescence lifetime using the phasor approach. [SPCImage](https://www.becker-hickl.com/products/spcimage/) is a commercial software that provides a user interface to extract pixel-level fitted and phasor features. [Prism](https://www.graphpad.com/prism/) is a commercial software that allows users to visualize and perform statistical analysis. 

However, while some tools offer code-free interfaces, users still need to write custom code—either to prepare data in the proper format as input, or to further process their outputs for downstream analysis. The fragmentation between tools arises from their focus on only a subset of data levels: <span style="color: #1f77b4;">pixel</span>, <span style="color: #ff7f0e;">cell ROI</span>, <span style="color: #2ca02c;">channel</span> , <span style="color: #d62728;">field of view</span>, and <span style="color: #9467bd;">experiment</span>. 
