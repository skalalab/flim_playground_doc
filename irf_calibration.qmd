...
The app evaluates these conditions separately for each channel. Estimate and confirm shifts for the channels that meet them. Each required channel appears in its own initially open **shift calibration** expander, which can be collapsed after inspection. Then continue to [Numerical Feature Extraction](numerical_feature_extraction.qmd#feature-extraction).

A calibration dye is a **Fluorescence Lifetime Standard**: its reference-based phasor calibration runs during [fit-free feature extraction](lifetime_fit_free.qmd#fit-free-standard-calibration). It does not trigger this IRF shift stage. A workflow with only intensity-only channels also proceeds directly to extraction. [QPI background correction](qpi.qmd#background-correction) has its own per-channel calibration in the same Numerical workflow.

## Input

Open **Data Extraction → Numerical**, select the source folder, and review its [file and acquisition checks](fov_metadata.qmd). Click **Start calibration**. The app automatically saves the metadata, then presents the calibration controls. For FLIM channels, review the fitting options and click **Optimize for Shifts**; this button reads **Calibrate channels** when QPI background correction is also required.
...
The high-SNR curves are gathered according to the [decay type](data_extraction_config.qmd#decay-types). For [2D input](data_extraction_config.qmd#two-d-decay), the selection targets approximately 30 curves, with at least one per FOV (one CSV file). It prefers the brightest curves below `100000` photons; if a FOV has none below that threshold, it uses its brightest curves without the cap. For [3D/4D input](data_extraction_config.qmd#three-d4d-decay), one curve per image sums the pixels inside its [ROI mask](data_extraction_config.qmd#mask).
...
Therefore, $n$ determines the parameters to be fitted: the amplitudes $A_i$ and the lifetimes $\tau_i$. $\mathbf{t}$ is the time axis of the decay curve calculated by the `duration` and `time bins` from the [decay info](fov_metadata.qmd#metadata-decay-info):
...
4. When satisfied, click **Confirm calibration for each channel**. Confirmation automatically updates the metadata file with the gates and shifts.
...
During shift estimation, `Fitting Mode` offers **Hybrid** only. After all required calibration has been confirmed, the fitting workflow offers both **Hybrid** and **Local** for the subsequent cell-level fits. Choose the mode before clicking `Start extraction`.
...
Once satisfied with the [fitting options](#fitting-options), time gates, and shift values for every required channel, click **Confirm calibration for each channel** below the channel expanders. The same control confirms QPI background settings when that calibration is included.

After confirmation, the app automatically updates the existing metadata file with the shifts, time gates, fitting options, component counts, and fixed lifetimes. Choose the fitting mode for raw FLIM channels using `Lifetime fit`, then click **Start extraction**, or use **Go back and find shift** to repeat calibration (**Go back and recalibrate** for a mixed FLIM/QPI workflow). A metadata-save error must be resolved with **Retry saving metadata** before extraction can run. See the [Numerical workflow](numerical_feature_extraction.qmd#calibration-interface) for source changes and automatic saving.

::: {#calibration-done}
![After confirming calibration, choose the fitting mode and start extraction.](extraction_ui_shots/calibration_done.png){width=80% fig-align=center fig-alt="Confirmed calibration with a fitting-mode selector and buttons to start extraction or return to shift calibration."}
:::
