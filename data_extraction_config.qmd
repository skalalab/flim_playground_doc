...
**✨ NEW ✨**: Users can now create, save, and switch between multiple data extraction profiles — each an independent, ready-to-go setup for a different experiment or dataset. Up to **10 profiles** are supported.

On the **Configuration** page:

- Select an existing profile from the `Profile` dropdown to load it.
- Create a new profile with `➕ Create` (give it a name, e.g. `experiment-B`).
- Delete the active profile with `🗑️ Delete` (disabled when only one profile remains).

![](extraction_ui_shots/config_profile.png){width=60% fig-align=center}
...
::: {.callout-note}
Every column in a 2D decay CSV must be a numeric time bin. If any non-numeric column is present (for example, a cell-label or annotation column), FLIM Playground rejects the file with an error indicating that all columns are expected to be numeric time bins. Remove such columns before extraction.
:::
...
Users can specify the laser frequency in GHz, up to 1 GHz. For example, 0.08 GHz equals 80 MHz. It will be used in the [phasor analysis](#sec-phasor-analysis). If you do not intend to use the phasor analysis, any value in that range will do. 
...
Users can optionally assign a fixed value (in ns) for specific lifetime components (when $n>1$, e.g., $t_1$, $t_2$). If a value is provided, it serves as the default fixed lifetime during the [interactive fitting phase](numerical_feature_extraction.qmd#fixed-lifetimes). If left blank (or set to 0), the lifetime acts as a free parameter to be optimized. This is particularly useful when analyzing fluorophores with prior knowledge of their lifetimes.
