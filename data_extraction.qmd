...
The user can [configure](data_extraction_config.qmd) the system once and apply it to future data. After configuration, the user can extract features from raw data using the following steps, inspired by the [categorization](#index.qmd#abstraction-level):

![](extraction_ui_shots/extraction_steps.png){width=30% fig-align=center}

1. [FOV Metadata Organization](fov_metadata.qmd): each experiment session consists of multiple field of views (FOVs). This steps helps users organize the metadata of the FOVs. 
2. [Numerical feature extraction](numerical_feature_extraction.qmd): it calibrates and then extracts single cell numerical features. 
...
- User then can upload the extracted datasets to perform [Data Analysis](data_analysis.qmd). 
...
- Depending on your chosen number of lifetime components, the other pixel-prefitted features files will be included and their names are inferred from the `t1` file, by replacing `t1` in the file name with `t2`, `t3`, `a1[%]`, etc. 
...
If the channel's [imaging modality](#imaging-modality) is `Intensity-only`, the user should provide a 2D intensity image in:
