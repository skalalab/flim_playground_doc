...
Users can [configure](data_extraction_config.qmd) the system once and apply it to future data. After configuration, users can extract features from raw data using the following steps, inspired by the [categorization](index.qmd#feature-classes):

![](extraction_ui_shots/extraction_steps.png){width=30% fig-align=center}

1. [FOV Metadata Organization (A)](fov_metadata.qmd): each experiment session consists of multiple field of views (FOVs). This steps helps users organize the metadata of the FOVs. 
2. [Numerical feature extraction (B)](numerical_feature_extraction.qmd): it calibrates and then extracts single cell numerical features. 
    ...
    - per channel feature extraction including:
...
3. [Categorical feature extraction (C)](categorical_feature_extraction.qmd): based on the FOV name, extract the categorical features such as the treatment, time point, etc.

- Users then can upload the extracted datasets to perform [Data Analysis](data_analysis.qmd). 
...
### SPCImage t1
If the decay type is [3D/4D pixel-prefitted](data_extraction_config.qmd#3d4d-pixel-prefitted), users can provide pixel-prefitted lifetime feature files. 

- `.asc`: a 2D array in spatial dimensions, with each (row, column) has the value of the lifetime feature (e.g. `t1`: the first component's lifetime) of that pixel, output from [SPCImage](https://www.becker-hickl.com/products/spcimage/). 
- Depending on your chosen number of lifetime components, the other pixel-prefitted feature files will be included and their names are inferred from the `t1` file, by replacing `t1` in the file name with `t2`, `t3`, `a1[%]`, etc. 

### Reference Dye
If the reference dye-based calibration is chosen, users are expected to provide the reference dye file. 

- `.tiff`/`.tif`: a 3D array, with one of them as the time dimension. 

### Intensity (2D)

If the channel's [imaging modality](#imaging-modality) is `Intensity-only`, users are expected to provide a 2D intensity image in:
