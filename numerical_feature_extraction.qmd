...
After confirming calibration, raw FLIM channels using `Lifetime fit` default to **Local** fitting. Leave it selected for faster cell-level fitting or change the mode using [the mode guidance](lifetime_fit.qmd#hybrid-over-local), then click **Start extraction**. To revisit calibration, use **Go back and find shift**, **Go back and correct background**, or **Go back and recalibrate**, as appropriate for the channels.

![After confirming calibration, choose the fitting mode and start extraction.](extraction_ui_shots/calibration_done.png){width=80% fig-align=center fig-alt="Confirmed calibration with Fitting Mode set to Local by default and buttons to start extraction or return to shift calibration."}
...
![Follow per-FOV progress and results during numerical extraction.](extraction_ui_shots/numerical_feature_extraction.png){width=100% fig-align=center fig-alt="Numerical workflow showing Fitting Mode set to Local by default, per-field-of-view extraction progress, and result badges."}
