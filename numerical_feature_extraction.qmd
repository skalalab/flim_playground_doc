...
- [Lifetime fit](lifetime_fit.qmd): fit exponential models to decay curves to estimate lifetimes and their fractional contributions by minimizing an objective function. 
...
Users can either upload a previously extracted metadata file (left), or use the cached metadata file just extracted in the [fov metadata extraction](fov_metadata.qmd) step (right). 
...
Similar to the [fov metadata extraction](fov_metadata.qmd#fov-metadata) step, FLIM Playground extracts the features and displays the extraction status for each field of view. 
