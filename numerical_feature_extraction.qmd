...
- [Lifetime fit](#sec-lifetime-fit): fit exponential models to decay curves to estimate lifetimes and their fractional contributions by minimizing an objective function. 
...
- [Intensity Texture](#sec-intensity-texture): texture features of single-cell ROI intensity images
...
Users can either upload a previously extracted metadata file (left), or use the cached metadata file just extracted in the [fov metadata extraction](#sec-fov-metadata) step (right). 
...
The high SNR decay curves are constructed automatically based on the [decay type](data_extraction_config.qmd#decay-types). If it is in [2D format](data_extraction_config.qmd#two-d-decay), a total of the brightest 30 curves that are below `100000` photons are selected and evenly distributed across all fields of view (one CSV file is considered as one FOV). If it is in [3D/4D format](data_extraction_config.qmd#three-d4d-decay), the fields of view are images, and one curve is constructed for each image that includes all the non-zero pixels within the [ROI mask](data_extraction.qmd#mask). 
...
##### Time Gates
Due to the dead time of the system certain time bins from the head and/or tail of the decay curve are not reliable. The T1 (head) and T2 (tail) gates are used to select the time range of the decay curve to be fitted. Users can inspect the decay curve by clicking the full-screen mode button of the plot on the right of the [shift result](#irf-shift). Hover-based interaction is implemented so users can see the time bin numbers to have a better sense of the time range. 
...
Weighted Least Squares (`WLS`) minimizes the squared residuals between the measured curve and the model prediction, with each time bin weighted by the inverse of its expected variance. Because photon counts follow Poisson statistics (variance $\approx$ count), each residual is scaled by $1/\sqrt{\max(y_n, 1)}$, so the cost minimized is
...
This weighting (equivalent to Pearson's $\chi^2$) keeps the high-count bins near the decay peak from dominating the fit, and the $\max(\cdot, 1)$ floor avoids division by zero in empty bins.

##### Fitting Mode

![](extraction_ui_shots/fitting_mode.png){width=20% fig-align=center}

**Fit calibration** (IRF shift) always runs **Hybrid** optimization to ensure accuracy. For **Lifetime fit** cell-level extraction after calibration, the fitting mode selects the optimization strategy. It is a trade-off between speed and the effort to avoid local minima.

- `Local`: It uses the `leastsq` (least squares with Levenberg-Marquardt) algorithm if the chosen metric is `WLS`, otherwise it uses the `nelder` (Nelder-Mead) algorithm.
- `Hybrid`: The slower but more accurate option that combines the `differential evolution` algorithm to obtain a good initial guess for all parameters, followed by the `Local` optimizer to refine the solution. 
...
Then, depending on the chosen [calibration method](data_extraction_config.qmd#calibration-method), the following two methods are available:
...
::: {.callout-note}
If both the `Lifetime fit` and `Lifetime fit free` feature extractors are selected for this channel, the optimized IRF shift derived from [the fit calibration](#fit-calibration) step is reused for the `Lifetime fit free` feature extractor. 
:::
...
Because the fluorescence lifetime standard is measured in the same system as the decay curves and we know its lifetime, there is no need to account for the IRF shift and the offset. Therefore, after subtracting the offset, `phasorpy`'s [`lifetime.phasor_calibrate`](https://www.phasorpy.org/docs/stable/api/lifetime/#phasorpy.lifetime.phasor_calibrate) function is used to calibrate all the decay curves behind the scenes when calculating the phasor coordinates. 

### Confirm Calibration

Once users are satisfied with the calibration settings ([fitting options](#fitting-options) and shift values), they can click the `Confirm Calibration` button at the bottom of the [page](#irf-shift). 
...
If the selected feature extractors do not require IRF shift calibration, or users have [finished the calibration](#calibration-done), they can proceed to the feature extraction step by clicking the `Confirm and Start Analysis` button. 

Similar to the [fov metadata extraction](#sec-fov-metadata) step, FLIM Playground extracts the features and displays the extraction status for each field of view. 
