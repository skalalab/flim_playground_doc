...
Calibration in FLIM is essential because raw decays are convolved with the instrument response function (IRF)—the timing profile of the detection system’s response to an ultrashort light pulse that broadens and shifts the measured decay—, so without correcting these, fitted lifetimes and phasor positions are biased and not comparable across days, samples, or instruments. 
...
In addition to shifting the IRF based on the chosen shift values, each decay curve will be subtracted an offset value and clip to 0 if the time bin is negative after the subtraction. The offset is chosen to be the mean of the last 10th percentile of the decay curve (tail). 
...
Warnings will be displayed if cells have `NaN` values for some features. They will *not* be excluded from the final csv file. 
