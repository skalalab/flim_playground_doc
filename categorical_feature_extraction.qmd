...
![This category-mapping example uses a table of 1,572 cells from 12 FOVs.](extraction_ui_shots/cat.png){width=100% fig-align=center fig-alt="Categorical Feature Extraction with a local dataset folder, underscore delimiter, category selections, slot mappings, and a preview of the assigned labels."}
...
![Map FOV-name slots to categories and check the preview before exporting.](extraction_ui_shots/cat_slot_assign.png){width=100% fig-align=center fig-alt="The MCF7, 2DG, and dish2 slots map to cell_line, treatment, and dish; a five-FOV preview appears above Confirm category mapping & export the combined dataset."}

Clicking `Confirm category mapping & export the combined dataset` then writes the merged dataset with the categorical features as a timestamped CSV (`{timestamp}_combined.csv`) into the same folder as the input dataset. Note that this is a direct file write rather than a browser download, so the file is saved on the machine running FLIM Playground. When using the [hosted demo](https://flim-playground.streamlit.app/), the file is written to the server and does not reach your computer, which is one reason Data Extraction is intended to be run on a local installation.
