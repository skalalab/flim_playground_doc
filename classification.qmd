...
All the classifiers are implemented using `scikit-learn`. The confusion matrix and the ROC curve come from `sklearn.metrics`; the remaining [performance metrics](#performance-metrics) are computed directly from that confusion matrix using the standard formulas given below, which avoids re-scanning the predictions once per metric and covers quantities such as specificity and Youden's J that `sklearn.metrics` does not provide. A fixed random seed (42) is used for all the classifiers to ensure reproducibility.
...
- Choose the numerical features on the left.
    - The z-score normalization is applied to the features before training if the chosen classifier is `SVM` or `Logistic Regression`, because they are sensitive to feature magnitudes, while `Random Forest` and `Gradient Boosting` are not. 
...
The classifier hyperparameter panel is organized per classifier with three roles:
...
| Param | Role | Effect | Priority |
| --- | --- | --- | --- |
| `regularization` | Regularization | Selects regularization type; available options depend on `solver` (see table below). Maps to `l1_ratio` internally. | High |
...
- The annotated operating point in <span style="color:red;">red</span> is "better" *after* the threshold tuning because it achieves higher Recall (TPR) + Specificity (TNR), as can be seen in the metrics table (the optimized metrics are highlighted in <span style="color:red;">red</span>). The threshold before was 0.5, the default value, and after tuning it was optimized to 0.19. The reduction in thresholds means that the classifier is tuned to be more "confident" at predicting the positive class (2DG), which is the minority class. The effect of this threshold optimization is also visible in the confusion matrix.
